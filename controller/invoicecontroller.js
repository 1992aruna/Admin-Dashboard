...
const addInvoice = async (req, res, next) => {
  ...
  const { 
    ...
    if (!invoice && req.user.role !== 'superadmin') {
      return res.status(500).json({ message: "Unauthorized user Unable To Add Invoice" });
    }
...
const deleteInvoice = async (req, res) => {
    ...
    try{
      ...
      return res.status(201).json({ message: "Invoice Successfully Deleted" });
}
